...
Traditional approaches of the wait time estimation rely on the analytic solution to the transient heat transfer in a semi-infinite medium:
...
\begin{equation}
T_{V,nuc} = T_{sat} + \frac{2\sigma T_{sat} \parth{\frac{1}{\rho_{V}} - \frac{1}{\rho_{L}} }}{R_{c}h_{LV}} 
\end{equation} 

The wait time is then assumed to be the time needed for the transient temperature field to reach $T_{V,nuc}$ at height $y=R_{c}$. Combining the two previous equations allow to write:

\begin{equation}
t_{w} = \frac{1}{4\eta_{L}}\crocht{ \frac{R_{c}}{\erfcrec{ \dfrac{\Delta T_{L}}{\Delta T_{L} + \Delta T_{w}} + \dfrac{2\sigma T_{sat} \parth{\dfrac{1}{\rho_{V}} - \dfrac{1}{\rho_{L}}} }{ \parth{\Delta T_{w} + \Delta T_{L}}h_{LV}R_{c} } } } }^{2} \approx \frac{1}{\pi \eta_{L}}\crocht{\frac{ \parth{\Delta T_{w} + \Delta T_{L}}R_{c} }{\Delta T_{w} - \dfrac{2\sigma T_{sat} \parth{\dfrac{1}{\rho_{V}} - \dfrac{1}{\rho_{L}}} }{R_{c} h_{LV}} } }^{2}
\end{equation}

This expression needs an estimation of the cavity radius where bubbles nucleate, for which the following expression is proposed by Gilman \cite{gilman_phd}:

\begin{equation}
R_{c} = \frac{2\sigma \parth{1 + \dfrac{\rho_{V}}{\rho_{L}} } / P}{\exp{h_{LV} \dfrac{\Delta T_{w}}{R_{g}T_{w}T_{sat}}} - 1}
\end{equation}


Similarly, Yeoh \etal \cite{yeoh_fundamental_2008} propose an improved expression that accounts for the contact angle value:

\begin{align}
t_{w} &= \frac{1}{\pi \eta_{L}}\crocht{ \frac{\Delta T_{L} C_{1} R_{c}}{\Delta T_{w} - \dfrac{2\sigma T_{sat}}{C_{2}\rho_{V}h_{LV}R_{c}}}  }^{2} \\
%
R_{c} &= \sqrt{\dfrac{1}{C_{1}C_{2}} \dfrac{2 \sigma T_{sat} \lambda_{L}}{\rho_{V}h_{LV}\phi_{w}}}\\
%
