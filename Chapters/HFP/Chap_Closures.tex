...
\begin{equation}
A_{c,V} = \parth{N_{coal,st} + \frac{N_{coal,sl}}{2}}\pi \parth{\frac{2}{3}R_{d}\ \sin{\theta}}^{2} + \frac{N_{coal,sl}}{2}\pi \parth{\frac{2}{3}R_{sl}\ \sin{\theta}}^{2}
\end{equation}
...
On Table \ref{tab:HFP_summary}, we gather the formulations and closure laws of the proposed Heat Flux Partitioning model.
...
$C_{AM,x}K^{2} \dfrac{\Ja_{w}^{2}}{\Pr_{L}} + \dfrac{1}{3}\frac{\Re_{b}}{\Fr} + \dfrac{1}{8}C_{D}\Re_{b} > \dfrac{1}{2} \dfrac{f_{C,x}\parth{\theta, \dtheta}}{\Ca}$\newline
...
\\
%
\multirow{2}*{Sliding Velocity $U_{b}$} & {
\begin{align*}
...
\\
%
\multirow{6}*{Nucleation Site Density $N_{sit}$} & {
...
%
&1-\cos{\theta} = \parth{1-\cos{\theta_{0}}}\parth{\dfrac{T_{c}-T_{sat}}{T_{c}-T_{0}}}^{\gamma}
\end{align*}
...
$N_{sit,a} = \dfrac{\mathcal{W}\parth{N_{sit}A_{sit}}}{A_{sit}}$, $A_{sit} = t_{g,d} f \pi R_{d}^{2}$
}\\
\\
%%
\end{tabular}
\caption{Summary of the HFP model and its closure laws}
\label{tab:HFP_summary}.
\end{table}
