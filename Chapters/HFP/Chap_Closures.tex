...
\caption{Bubble wait time data in vertical flow boiling. Wall superheat for Richenderfer data are estimated using Frost \& Dzakowic correlation (Eq. \ref{eq:frost}).}
...
\caption{Evolution of the product $t_{w} \times f$ with the reduced Jakob number. Trend line is $t_{w} f = \dfrac{1}{1+0.05\exp{35\Ja_{w}^{*}}}$.}
...
NSD correlations actually estimate the total number of sites where bubbles can nucleate on a surface. However, experimental observations showed that nucleation sites exhibit different behavior 

they do not represent how important a nucleation site will be in term of bubbles generation compared to another. 
