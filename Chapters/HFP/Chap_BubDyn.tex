...
\end{equation}
which falls back to $\mathcal{F}\parth{\Ja} \to \frac{\sqrt{3}}{\sqrt{\pi}}$ when $\Ja \gg 1$.
...
\begin{itemize}
\item $R\parth{t} = 2.5 \dfrac{\Ja}{\sqrt{\Pr_{L}}}\sqrt{\eta_{L}t}$ if $\lambda_{w} \gg \lambda_{L}$ ;
\item $R\parth{t} = \dfrac{2}{\sqrt{\pi}} \sqrt{\dfrac{\lambda_{w}\rho_{w}c_{p,w}}{\lambda_{L}\rho_{L}c_{p,L}}}$ if $\lambda_{w} \ll \lambda_{L}$
\end{itemize}
...
\begin{align}
\grad{T} \cdot \vect{e_{r}} = \dpartial{T}{r}\parth{R, \theta, \phi} \approx\frac{T_{\L}(y)-T_{sat}}{\delta_{b}}
\end{align}
...
\begin{align}
d^{2}Q_{b} \approx -\frac{\lambda_{L}}{\delta_{b}}\crocht{ \Delta T_{w}R^{2}\sin{\theta}-\frac{\Delta T_{w}+\Delta T_{L}}{\delta}R^{3}\crocht{ \cos{\theta}-\cos{\Theta} } \sin{\theta} } \mathrm{d}\theta \mathrm{d}\varphi
\end{align}

\npar
Then assuming that $\delta_{b}$ is constant between $t$ and $t+dt$, the total heat flux can be expressed:
...
To further evaluate the proposed model, we compare the result with experimental measurements of bubble radius in vertical boiling of water at atmospheric pressure by Maity \cite{maity_effect_2000}. The choice of $\delta$ is adapted to each case in order to observe the behavior of the model.
