...
To further discuss this question, we will try to consider the bubble lift-off as a single event and therefore try to attribute a given lift-off diameter $D_{lo}$ based on available experimental measurements.
...
\begin{itemize}
\item The reduced Jakob numbers of superheat and subcooling $\Ja_{w}^{*}$ and $\Ja_{L}^{*}$. With $\Ja_{*} = \dfrac{c_{p,L}\Delta T}{h_{LV}}$ which excludes the impact of pressure through the density ratio.
...
This difficulty has already been pointed out by Montout \cite{montout_phd} who faced difficulties in consistently using the force balance perpendicular to the wall for bubble lift-off.

To demonstrate this uncertainty, we present on Figure \ref{fig:pred_dlo_bdfy} the predictions achieved when the lift-off is associated to a positive force balance perpendicular to the wall (Eq. \ref{eq:bdf_y}). 
...
The results are greatly dispersed and do not seem to demonstrate any coherent trend with the experimental measurements. 
...
As an alternative way to estimate the lift-off / maximum bubble diameter $D_{lo}$, we propose a simple correlation based on non-dimensional numbers as:

\begin{equation}
\frac{D_{lo}}{L_{c}} = e^{8.779} \Pr_{L}^{-0.065} \parth{\frac{\rho_{L}}{\rho_{V}}}^{-0.374} {\Ja_{w}^{*}}^{1.188} \parth{1+\Ja_{L}}^{-6.853} \parth{1+\Re_{\tau}}^{-0.547}
\label{eq:correl_dlo}
\end{equation}
