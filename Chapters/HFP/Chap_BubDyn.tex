...
\begin{note*}{}
This formulation is used in Kommajosyula's HFP model
\end{note*}
...
The estimation of the bubble sliding velocity through an explicit correlation is difficult since it varies over the bubble lifetime. Therefore, some authors simply suppose that $U_{b} = U_{L}$ such as Gilman \& Baglietto.
\end{remark*}
...
This result was generalized by Scriven (1959) \cite{scriven_dynamics_1959} who derived:
...
&\text{Drag greater than buoyancy if:}\ \  \Re_{b}>\frac{16}{3}\frac{1}{C_{D}}\frac{\Eo}{\Ca}=\Re_{c} \tag{Bd. 3} \label{eq:DvsB}
\end{align}
