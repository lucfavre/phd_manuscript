...
\draw[->,>=stealth,gray!50!blue] plot[domain=0:3.2,smooth,xshift=109,yshift=3] ({(\x *pi) r}:{\rayspi*(1-\x/6)}) ;
\draw[->,>=stealth,gray!50!blue] plot[domain=0:3.2,smooth,xshift=119,yshift=3] ({(\x *pi) r}:{\rayspi*(1-\x/6)}) ;
...
%\draw[dotted, green!50!black, opacity=0.7, line width = 3pt] (Oarcb) arc({-pi+(1.15*\thetrecrad)) r}:{0.3 r}:1.01*\rayrec) arc ({0.3 r}:{ 0.93*(pi-(\thetadvrad)) r}:1.01*\rayadv)--cycle;
%
%%\draw[dotted, green!50!black, opacity=0.7, line width = 2pt] (Oarcb) arc({-pi+(\thetrecrad)) r}:{0.1 r}:0.95*\rayrec) arc ({0.1 r}:{pi-(1.6*\thetadvrad)) r}:1.1*\rayadv) -- cycle;
%
%%%Upstream angle
%%\draw (Oarc) --++(-90+\thetrecrad r:0.3) node[very near end, below]{$\theta + \dtheta$};
%%\draw ($(Oarc)+(0,-0.15)$) arc(-90:-90+\thetrecrad r:0.15) ;
...
%%%CONTROL VOLMUE
%
%%\draw[densely dotted, green!50!black, very thick] ($(Oarc)+(0.03,-0.7*\ray)$)--($(Oarc2)+(0.03,\ray)$)--++(2*\ray,0)--($(Oarc)+(0.04+2*\ray,-0.7*\ray)$)--cycle;
%
%
%
%%Center and radius
...
\beginpgfgraphicnamed{static_coal}
...
\beginpgfgraphicnamed{static_suppression}
...
\draw[fill=white] ($(cavC) + (1.0,0.7)$) circle(0.5);
...
\coordinate (Oarc) at ($(Ob)+(\rw,0)$);
\shade[ball color = gray!5!white, opacity = 0.5] (Oarc) arc({-(pi/2-\alphrad) r}:{(pi+pi/2-\alphrad) r}:\ray);
...
\shade[ball color = gray!5!white, opacity = 0.5] (Ob2) circle(\ray);
\draw (Ob2) circle(\ray);
